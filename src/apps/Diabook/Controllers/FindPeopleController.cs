...
namespace Diabook.Controllers
{
    ...
    public class FindPeopleController : ApiController
    {
        ...
        {
            var client = new GraphClient(new Uri("http://neo4j:Temporary123@localhost:7474/db/data"));
            client.Connect();
        ...
        public IEnumerable<Person> Get(string name, int degreeOfSeparation)
        {
            ...
            client.Connect();
            var query = client.Cypher
                .Match(string.Format("(user:Person)-[:FRIENDS_WITH*{0}..5]->(fof)-[:DIAGNOSED_WITH]->(disease)", degreeOfSeparation))
                .Where((Person user) => user.Name == name)
